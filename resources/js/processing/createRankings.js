...
let standings = JSON.parse(fs.readFileSync('standings.json', 'utf8'));
...
function sortSimple(rankings){
	...
	fs.appendFile("rankings.json", "[", function(err) {if(err) {return console.log(err);} });

	//Print to see results
	for (var i = rankings.length - 1; i >= 0; i--) {
		fs.appendFile("rankings.json", "{\"fullName\": \"" + rankings[i]["fullName"] + 
			"\", \"wins\": " + 
			rankings[i]["win"] + 
			", \"losses\": " + 
			rankings[i]["loss"] +
